"""AgenC Worker — Sentiment Analyst (asyncio, N-worker collaboration)."""
...
from worker_tools.local_registry import capability_manifest_for, tools_for_sentiment_analyst
...
CAPABILITIES = capability_manifest_for("sentiment")
SYSTEM_PROMPT = (
    "You are a Sentiment Analyst agent on the AgenC decentralized network. "
    "You specialize in reading market psychology, social signals, and crowd behavior in crypto. "
    "Use your tools to fetch the Fear & Greed Index, trending coins, and global market overview — "
    "then synthesize a clear, opinionated sentiment picture. "
    "Cite actual index values and trending tickers. Keep responses punchy and actionable."
)
MOCK_RESULT = "MOCK: Fear & Greed at 72 (Greed). ETH trending #2. Market narrative: risk-on with rotation from BTC to altcoins."
...
_CLAIM_JSON_INSTRUCTION = (
    ...
    "should_claim TRUE when the task involves market sentiment, crowd psychology, social trends, "
    "fear & greed, trending coins, narrative analysis, investor mood, or interpreting WHY markets "
    "are moving (not just what the numbers are). "
    "For purely quantitative tasks (raw price feeds, Uniswap TVL, smart contract data), set "
    "should_claim TRUE with fit_score 0.3-0.5 if you can add sentiment context; only decline "
    "when there is zero sentiment or narrative angle.\n"
...
def evaluate_claim(task: str, bounty_id: str | None = None) -> dict:
    sid = new_stream_id()
    if MOCK_MODE:
        raw = '{"should_claim": true, "fit_score": 0.80, "claim_rationale": "MOCK: sentiment and market psychology angle."}'
...
def process_task_with_prompt(task: str, system_prompt: str, bounty_id: str | None = None) -> str:
    ...
    tools = tools_for_sentiment_analyst(WORKER_API)
...
async def handle_collaboration(payload: dict, fallback_emitter_peer_id: str) -> None:
    ...
    if is_lead:
        collab_prompt = SYSTEM_PROMPT + (
            ...
            f"using keys: {collab_memory_hint('sentiment')}. "
            f"You may read peer context from: {collab_read_hint('sentiment')}."
        )
    else:
        collab_prompt = SYSTEM_PROMPT + (
            f"\nIMPORTANT: You are collaborating with {', '.join(peer_specialties)}. "
            f"Read any data they stored using shared_memory_get (try keys: {collab_read_hint('sentiment')}). "
            f"Then provide your sentiment analysis incorporating that context. "
            f"Use your tools (fear_greed_index, crypto_trending, global_market_overview) and store "
            f"your findings under keys: {collab_memory_hint('sentiment')}."
        )
