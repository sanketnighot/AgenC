"""AgenC Worker — Data Analyst (asyncio, N-worker collaboration)."""
...
async def handle_collaboration(payload: dict, fallback_emitter_peer_id: str) -> None:
    ...
    # peer_workers: all collaborators OTHER than self
    peer_workers = payload.get("peer_workers", [])
    # Backward compat with old single-peer bridge format
    ...
    if is_lead:
        # Register share queue BEFORE any awaits so fast non-leads don't miss the window
        ...
        # Non-lead: send share directly to lead via P2P mesh
        ...
        # Notify bridge for UI animation
...
async def handle_new_bounty(from_peer: str, payload: dict) -> None:
    ...
    try:
        ...
        # Register future BEFORE sending CLAIM to prevent decision arriving before we listen
