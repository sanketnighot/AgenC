...
def process_task(task: str) -> str:
    if MOCK_MODE:
        return "MOCK: ETH showed 0.72 correlation to CPI over 5 years based on historical data."
    try:
        resp = client.chat.completions.create(
            model=MODEL,
            messages=[
    ...
    while time.time() < deadline:
        ...
        if payload:
            ...
            if payload.get("bounty_id") == bounty_id:
                ...
                # Different bounty — push back into a local buffer isn't possible
                # over AXL; just log and discard (we drain the queue)
